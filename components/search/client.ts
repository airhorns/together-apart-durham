...
export const pathToSearchState = (path: string) =>
  path.includes("?") ? paramsToSearchState(qs.parse(path.substring(path.indexOf("?") + 1))) : {};

export const searchStateToURL = (searchState: any) => (searchState ? `${window.location.pathname}?${qs.stringify(searchState)}` : "");
